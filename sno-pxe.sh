...
### 1. Initial checks 
if [ $# -lt 1 ]; then
    echo "./sno-pxe.sh <release image> [pull secret path]"
    echo "Usage example:"
    echo "$ ./sno-pxe.sh quay.io/openshift-release-dev/ocp-release:4.14.3-x86_64 # This works if REGISTRY_AUTH_FILE is already set"
    echo "$ ./sno-pxe.sh quay.io/openshift-release-dev/ocp-release:4.14.3-x86_64 ~/config/my-pull-secret"
...
<network>
  ...
  <ip address="192.168.100.1" netmask="255.255.255.0">
    <dhcp>
      ...
      <bootp file="${pxeURL}/agent.x86_64.ipxe"/>
    </dhcp>
  </ip>
</network>
EOF
...
imageContentSources:
- mirrors:
  - ${quayRegistry}/openshift/release
  source: quay.io/openshift-release-dev/ocp-v4.0-art-dev
- mirrors:
  - ${quayRegistry}/openshift/release-images
  source: quay.io/openshift-release-dev/ocp-release
additionalTrustBundle: |
EOF
# Check if the certificate file exists
if [ ! -f "$CAPath" ]; then
  echo "Error: Certificate file not found at $CAPath"
  exit 1
fi

# Read the certificate content
certificate_content=$(cat "$CAPath")

# Indent each line of the certificate and append to the file
while IFS= read -r line; do
  echo "  $line" >> "${assets_dir}/install-config.yaml"
done <<< "$certificate_content"
...
cd ${assets_dir} && ./openshift-install agent create pxe-files

python -m http.server 8000 --directory ${assets_dir}/boot-artifacts &
SERVER_PID=$!
